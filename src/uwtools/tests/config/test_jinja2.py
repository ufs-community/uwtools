...
def test_config_jinja2_dereference__local_values():
...
def test_config_jinja2_dereference__no_op(val):
...
def test_config_jinja2_dereference__no_op_due_to_error(logged, logmsg, val):
...
def test_config_jinja2_dereference__remove(logged):
...
def test_config_jinja2_dereference__str_expression_rendered():
...
def test_config_jinja2_render__dry_run(logged, template_file, values_file):
    expected = "roses are red, violets are blue"
...
def test_config_jinja2_render__fail(logged, tmp_path):
...
def test_config_jinja2_render__values_missing(logged, template_file, values_file):
...
def test_config_jinja2_render__values_needed(logged, template_file, values_file):
...
def test_config_jinja2_unrendered(s, status):
...
def test_config_jinja2__deref_convert(converted, logged, tag, value):
...
def test_config_jinja2__deref_convert__fail(logged, tag, value):
    loader = yaml.SafeLoader(os.devnull)
    val = UWYAMLConvert(loader, yaml.ScalarNode(tag=tag, value=value))
...
def test_config_jinja2__deref_render__held(logged):
...
def test_config_jinja2__deref_render(deref_render_assets, logged):
...
def test_config_jinja2__deref_render__fail(deref_render_assets, logged):
...
def test_config_jinja2__deref_render__unloadable_val(logged):
    val = "&XMLENTITY;"
...
def test_config_jinja2__log_missing_values(logged):
...
def test_config_jinja2__register_filters__env():
...
def test_config_jinja2__register_filters__path_join(key):
...
def test_config_jinja2__report(logged):
    expected = """
...
def test_config_jinja2__supplement_values():
    assert jinja2._supplement_values() == {}


def test_config_jinja2__supplement_values__dict(supplemental_values):
    sv = supplemental_values
    assert jinja2._supplement_values(values_src=sv.d) == sv.d


def test_config_jinja2__supplement_values__dict_plus_env(supplemental_values):
    sv = supplemental_values
    with patch.dict(os.environ, sv.e, clear=True):
        assert jinja2._supplement_values(values_src=sv.d, env=True) == {**sv.d, **sv.e}


def test_config_jinja2__supplement_values__dict_plus_env_plus_overrides(supplemental_values):
    sv = supplemental_values
    with patch.dict(os.environ, sv.e, clear=True):
...
def test_config_jinja2__supplement_values__dict_plus_overrides(supplemental_values):
    sv = supplemental_values
    assert jinja2._supplement_values(values_src=sv.d, overrides=sv.o) == {**sv.d, **sv.o}


def test_config_jinja2__supplement_values__env(supplemental_values):
    sv = supplemental_values
    with patch.dict(os.environ, sv.e, clear=True):
        assert jinja2._supplement_values(env=True) == sv.e


def test_config_jinja2__supplement_values__env_plus_overrides(supplemental_values):
    sv = supplemental_values
    with patch.dict(os.environ, sv.e, clear=True):
        assert jinja2._supplement_values(env=True, overrides=sv.o) == {**sv.o, **sv.e}


def test_config_jinja2__supplement_values__overrides(supplemental_values):
    sv = supplemental_values
    assert jinja2._supplement_values(overrides=sv.o) == sv.o


def test_config_jinja2__supplement_values__file(supplemental_values):
    sv = supplemental_values
    assert jinja2._supplement_values(values_src=sv.f) == sv.d


def test_config_jinja2__supplement_values__file_plus_env(supplemental_values):
    sv = supplemental_values
    with patch.dict(os.environ, sv.e, clear=True):
        assert jinja2._supplement_values(values_src=sv.f, env=True) == {**sv.d, **sv.e}


def test_config_jinja2__supplement_values__file_plus_env_plus_overrides(supplemental_values):
    sv = supplemental_values
    with patch.dict(os.environ, sv.e, clear=True):
...
def test_config_jinja2__supplement_values__file_plus_overrides(supplemental_values):
    sv = supplemental_values
    assert jinja2._supplement_values(values_src=sv.d, overrides=sv.o) == {**sv.d, **sv.o}


def test_config_jinja2__supplement_values__priority(supplemental_values):
...
def test_config_jinja2__values_needed(logged):
...
def test_config_jinja2__write_template__to_file(tmp_path):
    outfile = tmp_path / "out.txt"
...
def test_config_jinja2__write_template__to_stdout(capsys):
