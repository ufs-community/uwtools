...
def test_FileStager(assets):
    ...
    stager = file.FileStager(target_dir=dstdir, config=cfgfile, keys=["a", "b"])
...
def test_FileStager_bad_key(assets):
    ...
    with raises(UWConfigError) as e:
        file.FileStager(target_dir=dstdir, config=cfgfile, keys=["a", "x"])
...
def test_FileCopier(assets):
    ...
    stager = file.FileCopier(target_dir=dstdir, config=cfgfile, keys=["a", "b"])
    stager.go()
...
def test_FileLinker(assets):
    ...
    assert not (dstdir / "foo").exists()
    assert not (dstdir / "subdir" / "bar").exists()
    stager = file.FileLinker(target_dir=dstdir, config=cfgfile, keys=["a", "b"])
    stager.go()
