...
def test_api_make_execute(execute_kwargs):
...
@pytest.mark.parametrize("val", [Path("/some/path"), {"foo": 88}])
def test_api_str2path_passthrough(val):
    assert api.str2path(val) == val


def test_api_str2path_convert():
    val = "/some/path"
...
def test_api__execute(execute_kwargs, tmp_path):
