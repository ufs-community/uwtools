...
def test_ensure_config_passthrough(val):
    assert api.ensure_config(val=val, stdin_ok=False) == val


def test_ensure_config_stdin_not_ok():
    with raises(UWError) as e:
        api.ensure_config(val=None, stdin_ok=False)
    assert str(e.value) == "Set stdin_ok=True to permit read from stdin"


def test_ensure_config_stdin_ok():
    assert api.ensure_config(val=None, stdin_ok=True) is None


def test_ensure_config_str_to_path():
    val = "/some/path"
    result = api.ensure_config(val=val, stdin_ok=False)
