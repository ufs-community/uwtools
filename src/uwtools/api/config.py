...
def realize(
    ...
    When a filename is available for an input or output, its format will be deduced from its
    extension, if possible. This can be overriden by specifying the format explicitly, and it is
    required to do so for reads from stdin or writes to stdout, as no attempt is made to deduce the
...
def realize_to_dict(
    ...
    When a filename is available for an input, its format will be deduced from its extension, if
    possible. This can be overriden by specifying the format explicitly, and it is required to do so
    for reads from stdin, as no attempt is made to deduce the format of streamed data.
...
def translate(
    ...
    Currently supports atparse -> Jinja2 translation, in which @[] tokens are replaced with Jinja2
    {{}} equivalents. Specify input_format=atparse and output_format=jinja2.

    If no input file is specified, stdin is read. If no output file is specified, stdout is written
    to. In dry_run mode, output is written to stderr.
...
def validate(
    ...
    If no config is specified, stdin is read and will be parsed as YAML and then validated. A dict
    or a YAMLConfig instance may also be provided for validation.
