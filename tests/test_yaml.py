...
yaml_config = config.YAMLConfig(PurePath().joinpath(uwtools_pwd,"fixtures/experiment.yaml"))
yaml_config2 = config.YAMLConfig(PurePath().joinpath(uwtools_pwd,"fixtures/gfs.yaml"))
...
def test_yaml_parse_env():
    ...
    yaml_config_env = yaml_config3.parse_include(yaml_config.config_path, from_environment=True)
...
def test_yaml_update():
    ...
    yaml_config_up=config.YAMLConfig(PurePath().joinpath(uwtools_pwd,'fixtures/experiment.yaml'))
    yaml_config_up.parse_include(config_file=PurePath().joinpath(uwtools_pwd,'fixtures/gfs.yaml'))

    expected =  "/home/myexpid/{{current_cycle}}"
    actual = yaml_config_up.datapath
...
def test_yaml_update_object():
    ...
    yaml_config.parse_include(data=yaml_config2.config_obj)
    expected =  "/home/myexpid/{{current_cycle}}"
...
def test_configuration_realtime_update():
    ...
    yaml_config.parse_include(PurePath().joinpath(uwtools_pwd,
...
def test_configuration_config_path():
    ...
    yaml_config_chk = config.YAMLConfig(PurePath().joinpath(uwtools_pwd,'fixtures/experiment.yaml'))
    name = PurePath(yaml_config_chk.config_path).name
