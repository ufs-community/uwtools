...
def test_template_translate(tb):
    assert tb.cell_output_text(23) == (base / "translate-template.yaml").read_text().strip()
    assert tb.cell_output_text(25) == "True"
    assert tb.cell_output_text(27) == (base / "complete-translate.yaml").read_text().strip()
